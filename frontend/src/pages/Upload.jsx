...
export function Upload() {
  ...
  const validateFile = (selectedFile) => {
    ...
    if (!allowedExtensions.includes(fileExt)) {
      setError(`Unsupported format (${fileExt}). Please select an MP3, WAV, M4A, or MP4 file.`);
      return false;
    }

    if (selectedFile.size > maxSizeBytes) {
      setError(`File size (${formatFileSize(selectedFile.size)}) exceeds the 100MB limit.`);
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      toast.success('Audio uploaded successfully. Opening workspace...');
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <AnimatePresence mode="wait">
            {!file ? (
              ...
              <motion.div
                ...
                exit={{ opacity: 0 }}
                className="p-4 rounded-xl bg-slate-950/60 border border-emerald-500/30 flex items-center justify-between gap-4"
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="text-left min-w-0">
                    ...
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      {formatFileSize(file.size)} · <span className="text-emerald-400">Ready to upload</span>
                    </p>
                  </div>
                </div>
        ...
        {file && (
          <GlassCard className="p-5 space-y-2">
            <label htmlFor="meeting-title" className="block text-xs font-medium text-slate-300">
              Meeting Title
            </label>
            <input
              id="meeting-title"
        ...
        {success && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span>Audio uploaded successfully. Opening workspace...</span>
            </div>
        ...
        {file && !success && (
          <div className="flex justify-end gap-2.5 pt-1">
            ...
            <Button
              ...
              icon={UploadIcon}
            >
              {uploading ? 'Uploading Audio...' : 'Upload Recording'}
            </Button>
          </div>
        )}
      </form>
